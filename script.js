...
const RAW_BASE = `https://raw.githubusercontent.com/${REPO_USER}/${REPO_NAME}/master`;
...
async function getImage(folder) {
  try {
    const res = await fetch(`${API_URL}/${folder}/images`, {
      headers: TOKEN ? { Authorization: `token ${TOKEN}` } : {},
    });
    if (!res.ok) return null;
    const files = await res.json();
    if (!files.length) return null;
    const img = files.sort((a, b) => a.name.localeCompare(b.name))[0];
    return `${RAW_BASE}/${folder}/images/${img.name}`;
  } catch {
    return null;
  }
}
...
async function load() {
  ...
  try {
    ...
    const folders = items
      .filter((f) => f.type === "dir" && f.name !== "0_main")
    ...
    const projects = await Promise.all(
      folders.map(async (f) => ({
        ...
        image: await getImage(f.name),
      }))
    );

    render(projects);
  } catch {
    grid.innerHTML = `<p style="color:#ff6b6b;text-align:center;padding:4rem;">Failed to load projects.</p>`;
  } finally {
...
function render(projects) {
  grid.innerHTML = projects
    .map((p) => {
      ...
      const github = `https://github.com/${REPO_USER}/${REPO_NAME}/tree/master/${p.name}`;
      ...
      <div class="card" data-title="${p.title.toLowerCase()}">
        ...
        <div class="card-buttons">
          <a href="${codepen}" target="_blank" class="btn codepen-btn" title="Preview in CodePen">
          </a>
          <a href="${github}" target="_blank" class="btn github-btn" title="View Source on GitHub">
          </a>
        </div>
      </div>
    `;
